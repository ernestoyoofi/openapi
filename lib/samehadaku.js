...
async function RecommendStreaming({ length } = {}) {
  ...
  $("#container #main ul li").each((i, el) => {
    listwatch.push({
      ...
      slug: new URL($(".thumb a", el).attr("href")).pathname.split("/")[1],
      type: "cardlisted"
    })
  })
...
async function GetDetailStreaming({ slug }) {
  ...
  return {
    ...
    title: $("head title").text()?.trim()?.split("-")[0]?.slice(6)?.trim(),
    desc: $(".infoanime.widget_senction .infox .desc p").text()?.trim(),
    status: $(".anim-senct .right-senc.widget_senction span:nth-child(4)").text()?.trim(),
