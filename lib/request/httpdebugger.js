...
async function RequestApi(url, options) {
  try {
    ...
    const makerequest = await axios.post(nativeURL, formPost, { headers })
    const results = makerequest.data
    ...
    $("#content > div > div > div > div:nth-child(4) > div > div:nth-child(4) > div.row.tools-table").each((i, el) => {
      resultsHeader[$(".col-sm-2", el).text()?.trim()?.toLowerCase()] = $(".col-sm-10", el).text().trim()
    })
    return {
