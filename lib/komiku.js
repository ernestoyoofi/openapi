...
async function Manga_Recommend({ length }) {
  ...
  $("div.bge").each((i, el) => {
    list.push({
      ...
      rec: {
        label: $(".kan .new1 a",el)?.eq(1)?.text()?.trim()?.split(":")[1],
        slug: $(".kan .new1 a",el)?.eq(1)?.attr("href")?.split("/")[1],
      },
...
async function Manga_Read({ slug, search, length = 0 }) {
  ...
  const getChapter = (typeof search == "string" && search.length > 7)? ifg.chapter?.filter(a => !!a.slug.match(search))[0]?.key || 0 : isNaN(length)? 0 : Number(length)
  const iLg = getChapter
  if(ifg.notFound) {
  ...
  return {
    detail: {
      slug: ifg.chapter[iLg].slug,
      openweb: `${baseURL.read+ifg.chapter[iLg].slug}/`
    },
    title: ifg.title,
    subtitle: ifg.subtitle,
    chapter: iLg+1,
    islast: iLg > (ifg.totalchapter - 2),
    image: list_image
  }
}
